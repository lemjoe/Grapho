...
function getNewValue(button, text) {
  ...
  if (selectedText !== '') {
    if (wrap) {
      ...
      const wrappedText = wrapText(selectedText, value);
      if (value === '~~~') return [text.replace(selectedText, wrapText('\n'+selectedText+'\n', value, false)), currentTextPosition + (value.length * 2 + 3)];
      if (value === 'link') return [text.replace(selectedText, `[` + selectedText + `]()`), currentTextPosition + 2];
      return [text.replace(selectedText, `[` + selectedText + `]()`), currentTextPosition + value.length * 2];
    } else {
